...
<div class="container-fluid">
    <div class="pagetitle">
        <div class="row justify-content-between">
            <div class="col">
                <h1>Rekapitulasi | <small>Pegawai Bedasarkan Agama</small></h1>
                <nav>
                    <ol class="breadcrumb">
    ...
    </div><!-- End Rekapitulasi Agama Title -->

    {{-- Notifikasi jika ada pegawai tanpa Agama --}}
    @if ($pegawaiTanpaAgama > 0)
        <div class="alert alert-success" role="alert">
            <i class="bi bi-info-circle"></i> 
            <strong>Perhatian.</strong> Terdapat <strong>{{ $pegawaiTanpaAgama }}</strong> data pegawai tidak dilengkapi dengan Agama. 
            <a href="#" class="alert-link" data-bs-toggle="modal" data-bs-target="#modalPegawaiTanpaAgama">Lihat detail</a>
        </div>
        <!-- Modal -->
        <div class="modal fade" id="modalPegawaiTanpaAgama" tabindex="-1" aria-labelledby="modalPegawaiTanpaAgamaLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modalPegawaiTanpaAgamaLabel">Detail Pegawai Tanpa Agama</h5>
                    ...
                    <div class="modal-body">
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @foreach ($dataPegawaiTanpaAgama as $index => $pegawai)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $pegawai->nip }}</td>
                                        <td>{{ $pegawai->nama }}</td>
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        const categories = {!! json_encode($rekap->pluck('agama')) !!};
        ...
        new ApexCharts(document.querySelector("#chartAgama"), {
            chart: {
                type: 'bar',
                height: 500,
            ...
            xaxis: {
                categories: ['Agama'] // Hanya satu kategori (untuk semua jenis kepegawaian)
            },
            yaxis: {
                title: {
                    text: 'Jumlah'
