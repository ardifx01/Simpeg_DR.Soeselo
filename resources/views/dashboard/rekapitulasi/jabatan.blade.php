...
<div class="container-fluid">
    <div class="pagetitle">
        <div class="row justify-content-between">
            <div class="col">
                <h1>Rekapitulasi | <small>Pegawai Bedasarkan Jabatan</small></h1>
                <nav>
                    <ol class="breadcrumb">
    ...
    </div><!-- End Rekapitulasi Jabatan Title -->

    {{-- Notifikasi jika ada pegawai tanpa Jabatan --}}
    @if ($pegawaiTanpaJabatan > 0)
        <div class="alert alert-success" role="alert">
            <i class="bi bi-info-circle"></i> 
            <strong>Perhatian.</strong> Terdapat <strong>{{ $pegawaiTanpaJabatan }}</strong> data pegawai tidak dilengkapi dengan Jabatan. 
            <a href="#" class="alert-link" data-bs-toggle="modal" data-bs-target="#modalPegawaiTanpaJabatan">Lihat detail</a>
        </div>
        <!-- Modal -->
        <div class="modal fade" id="modalPegawaiTanpaJabatan" tabindex="-1" aria-labelledby="modalPegawaiTanpaJabatanLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modalPegawaiTanpaJabatanLabel">Detail Pegawai Tanpa Jabatan</h5>
                    ...
                    <div class="modal-body">
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @foreach ($dataPegawaiTanpaJabatan as $index => $pegawai)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                                        <td>{{ $pegawai->nip }}</td>
                                        <td>{{ $pegawai->nama }}</td>
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        const categories = {!! json_encode($rekap->pluck('nama')) !!};
        ...
        new ApexCharts(document.querySelector("#chartJabatan"), {
            chart: {
                type: 'bar',
                height: 500,
            ...
            xaxis: {
                categories: ['Jabatan'] // Hanya satu kategori (untuk semua jenis kepegawaian)
            },
            yaxis: {
                title: {
                    text: 'Jumlah'
